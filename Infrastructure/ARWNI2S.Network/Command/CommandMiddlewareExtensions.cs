...
namespace ARWNI2S.Engine.Network.Command
{
    public static class CommandMiddlewareExtensions
    {
            ...
            where TPackageInfo : class
        {
            ...
            var useCommandMethod = typeof(CommandMiddlewareExtensions).GetMethod("UseCommand", new Type[] { typeof(INodeServerHostBuilder) });
            useCommandMethod = useCommandMethod.MakeGenericMethod(keyType, typeof(TPackageInfo));

            var hostBuilder = useCommandMethod.Invoke(null, new object[] { builder }) as INodeServerHostBuilder;
