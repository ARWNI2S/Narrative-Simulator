...
namespace ARWNI2S.Engine.Options
{
    ...
    /// </summary>
    public class EngineCoreOptions
    {
        static EngineCoreOptions()
        {
        ...
        /// </summary>
        public EngineCoreConfigurationLoader ConfigurationLoader { get; set; }
        ...
        /// </summary>
        /// <returns>A <see cref="EngineCoreConfigurationLoader"/> for configuring endpoints.</returns>
        public EngineCoreConfigurationLoader Configure() => Configure(new ConfigurationBuilder().Build());
        ...
        /// <returns>A <see cref="EngineCoreConfigurationLoader"/> for further endpoint configuration.</returns>
        public EngineCoreConfigurationLoader Configure(IConfiguration config) => Configure(config, reloadOnChange: false);
        ...
        /// </param>
        /// <returns>A <see cref="EngineCoreConfigurationLoader"/> for further endpoint configuration.</returns>
        public EngineCoreConfigurationLoader Configure(IConfiguration config, bool reloadOnChange)
        {
            if (EngineServices is null)
            {
                throw new InvalidOperationException($"{nameof(EngineServices)} must not be null. This is normally set automatically via {nameof(IConfigureOptions<EngineCoreOptions>)}.");
            }
            ...
            var loader = new EngineCoreConfigurationLoader();
