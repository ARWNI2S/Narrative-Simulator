...
{
    class ProxyV2PartReader<TPackageInfo> : ProxyPackagePartReader<TPackageInfo>
    {
        ...
        public override bool Process(TPackageInfo package, object filterContext, ref SequenceReader<byte> reader, out IPackagePartReader<TPackageInfo> nextPartReader, out bool needMoreData)
        {
            ...
            if (proxyInfo.AddressLength == 0)
            {
                ...
                proxyInfo.Version = versionAndCommand / 16;
                ...
                proxyInfo.AddressFamily = (addressFamilyAndProtocol / 16) switch
                {
                ...
                proxyInfo.ProtocolType = (addressFamilyAndProtocol % 16) switch
                {
            ...
            {
                reader.TryReadBigEndian(out uint sourceIpData);
                ...
                reader.TryReadBigEndian(out uint destinationIpData);
                ...
                reader.TryReadBigEndian(out ushort sourcePort);
                proxyInfo.SourcePort = sourcePort;

                reader.TryReadBigEndian(out ushort destinationPort);
