...
namespace ARWNI2S.Portal.Services.Blogs
{
    ...
    public partial class BlogService : IBlogService
    {
        ...
        /// Gets all blog posts
        /// </summary>
        /// <param name="nodeId">The server identifier; pass 0 to load all records</param>
        ...
        /// </returns>
        public virtual async Task<IPagedList<BlogPost>> GetAllBlogPostsAsync(int nodeId = 0, int languageId = 0,
        ...
        /// The task result contains the blog posts
        /// </returns>
        public virtual async Task<IPagedList<BlogPost>> GetAllBlogPostsByTagAsync(int nodeId = 0,
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            ...
            var blogPostsAll = await GetAllBlogPostsAsync(nodeId: nodeId, languageId: languageId, showHidden: showHidden);
        ...
        /// </returns>
        public virtual async Task<IList<BlogPostTag>> GetAllBlogPostTagsAsync(int nodeId, int languageId, bool showHidden = false)
        {
            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(BlogServiceDefaults.BlogTagsCacheKey, languageId, nodeId, showHidden);

            var blogPostTags = await _staticCacheManager.GetAsync(cacheKey, async () =>
            {
                var rezBlogPostTags = new List<BlogPostTag>();

                var blogPosts = await GetAllBlogPostsAsync(nodeId, languageId, showHidden: showHidden);
        ...
        public virtual async Task<IList<string>> ParseTagsAsync(BlogPost blogPost)
        {
            ...
            var tags = await blogPost.Tags.Split(separator, StringSplitOptions.RemoveEmptyEntries)
                .Select(tag => tag.Trim())
        ...
        /// </returns>
        public virtual async Task<IList<BlogComment>> GetAllCommentsAsync(int userId = 0, int nodeId = 0, int? blogPostId = null,
            bool? approved = null, DateTime? fromUtc = null, DateTime? toUtc = null, string commentText = null)
        {
            return await _blogCommentRepository.GetAllAsync(query =>
            {
                ...
                if (nodeId > 0)
                    query = query.Where(comment => comment.NodeId == nodeId);
        ...
        /// </returns>
        public virtual async Task<int> GetBlogCommentsCountAsync(BlogPost blogPost, int nodeId = 0, bool? isApproved = null)
        {
            ...
            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(BlogServiceDefaults.BlogCommentsNumberCacheKey, blogPost, nodeId, isApproved);
