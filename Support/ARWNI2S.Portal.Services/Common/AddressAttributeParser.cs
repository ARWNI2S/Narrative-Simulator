    ...
    public partial class AddressAttributeParser : IAddressAttributeParser
    {
        ...
        public virtual async Task<IList<string>> GetAttributeWarningsAsync(string attributesXml)
        {
            ...
            foreach (var a2 in attributes2)
            {
                ...
                //if not found
                var notFoundWarning = string.Format(await _localizationService.GetResourceAsync("Common.SelectAttribute"), await _localizationService.GetLocalizedAsync(a2, a => a.Name));
        ...
        public virtual async Task<string> ParseCustomAddressAttributesAsync(IFormCollection form)
        {
            ...
            {
                var controlId = string.Format(CommonServicesDefaults.AddressAttributeControlName, attribute.Id);
                ...
                switch (attribute.AttributeControlType)
                {
                    ...
                    case AttributeControlType.Checkboxes:
                        foreach (var attributeValue in attributeValues.ToString().Split(separator, StringSplitOptions.RemoveEmptyEntries))
                        {
