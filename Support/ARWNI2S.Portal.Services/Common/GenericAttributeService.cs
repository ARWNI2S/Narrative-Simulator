...
namespace ARWNI2S.Portal.Services.Common
{
    ...
    public partial class GenericAttributeService : IGenericAttributeService
    {
        ...
        /// <param name="nodeId">Node identifier; pass 0 if this attribute will be available for all servers</param>
        /// <returns>A task that represents the asynchronous operation</returns>
        public virtual async Task SaveAttributeAsync<TPropType>(BaseDataEntity entity, string key, TPropType value, int nodeId = 0)
        {
        ...
        /// <param name="entity">Entity</param>
        /// <param name="key">Key</param>
        /// <param name="nodeId">Load a value specific for a certain server; pass 0 to load a value shared for all servers</param>
        /// <param name="defaultValue">Default value</param>
        /// <returns>
        /// A task that represents the asynchronous operation
        /// The task result contains the attribute
        /// </returns>
        public virtual async Task<TPropType> GetAttributeAsync<TPropType>(BaseDataEntity entity, string key, int nodeId = 0, TPropType defaultValue = default)
        {
            ArgumentNullException.ThrowIfNull(entity);
            ...
            props = props.Where(x => x.NodeId == nodeId).ToList();
            if (!props.Any())
                return defaultValue;
        ...
        /// The task result contains the attribute
        /// </returns>
        public virtual async Task<TPropType> GetAttributeAsync<TEntity, TPropType>(int entityId, string key, int nodeId = 0, TPropType defaultValue = default)
            where TEntity : BaseDataEntity
        {
            var entity = (TEntity)Activator.CreateInstance(typeof(TEntity));
            entity.Id = entityId;

            return await GetAttributeAsync(entity, key, nodeId, defaultValue);
        }
