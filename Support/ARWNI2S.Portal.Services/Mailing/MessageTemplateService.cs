...
namespace ARWNI2S.Portal.Services.Mailing
{
    ...
    public partial class MessageTemplateService : IMessageTemplateService
    {
        ...
        /// <param name="nodeId">Node identifier; pass null to load all records</param>
        /// <returns>
        /// A task that represents the asynchronous operation
        /// The task result contains the list of message templates
        /// </returns>
        public virtual async Task<IList<MessageTemplate>> GetMessageTemplatesByNameAsync(string messageTemplateName, int? nodeId = null)
        {
            if (string.IsNullOrWhiteSpace(messageTemplateName))
                throw new ArgumentNullException(nameof(messageTemplateName));

            var key = _staticCacheManager.PrepareKeyForDefaultCache(MessageServicesDefaults.MessageTemplatesByNameCacheKey, messageTemplateName, nodeId);
            ...
            return await _staticCacheManager.GetAsync(key, async () =>
            {
                ...
                //filter by the server
                if (nodeId.HasValue && nodeId.Value > 0)
                    templates = await templates.WhereAwait(async messageTemplate => await _nodeMappingService.AuthorizeAsync(messageTemplate, nodeId.Value)).ToListAsync();
        ...
        /// </returns>
        public virtual async Task<IList<MessageTemplate>> GetAllMessageTemplatesAsync(int nodeId, string keywords = null, bool? isActive = null)
        {
            ...
            }, cache => cache.PrepareKeyForDefaultCache(MessageServicesDefaults.MessageTemplatesAllCacheKey, nodeId, isActive));
        ...
        public virtual async Task<MessageTemplate> CopyMessageTemplateAsync(MessageTemplate messageTemplate)
        {
            ...
            var mtCopy = new MessageTemplate
            {
                ...
                LimitedToNodes = messageTemplate.LimitedToNodes,
            ...
            //server mapping
            var selectedNodeIds = await _nodeMappingService.GetNodesIdsWithAccessAsync(messageTemplate);
            foreach (var id in selectedNodeIds)
                await _nodeMappingService.InsertNodeMappingAsync(mtCopy, id);
