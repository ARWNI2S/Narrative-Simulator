...
namespace ARWNI2S.Portal.Services.Messages
{
    /// <summary>
    /// Tokenizer
    /// </summary>
    public partial class Tokenizer : ITokenizer
    {
        ...
        {
            //define regex rules
            var regexFullConditionalSatement = TokenizerRegexFullConditionalSatement();
            var regexCondition = TokenizerRegexCondition();
        ...
        [GeneratedRegex("(?:(?'Group' %if)|(?'Condition-Group' endif%)|(?! (%if|endif%)).)*(?(Group)(?!))", RegexOptions.IgnoreCase | RegexOptions.Compiled | RegexOptions.Singleline | RegexOptions.IgnorePatternWhitespace, "es-ES")]
        private static partial Regex TokenizerRegexFullConditionalSatement();

        [GeneratedRegex("\\s*\\((?:(?'Group' \\()|(?'-Group' \\))|[^()])*(?(Group)(?!))\\)\\s*", RegexOptions.IgnoreCase | RegexOptions.Compiled | RegexOptions.Singleline | RegexOptions.IgnorePatternWhitespace, "es-ES")]
        private static partial Regex TokenizerRegexCondition();
