...
namespace ARWNI2S.Portal.Services.Media
{
    ...
    public partial class PictureService : IPictureService
    {
        ...
        //private readonly IRepository<TitlePicture> _titlePictureRepository;
        //private readonly IRepository<QuestPicture> _questPictureRepository;
        //private readonly IRepository<TournamentPicture> _tournamentPictureRepository;
            ...
            MediaSettings mediaSettings)
        {
            ...
            //_titlePictureRepository = titlePictureRepository;
            //_questPictureRepository = questPictureRepository;
            //_tournamentPictureRepository = tournamentPictureRepository;
        ...
        protected virtual async Task DeletePictureThumbsAsync(Picture picture)
        {
            ...
            var currentFiles = _fileProvider.GetFiles(_fileProvider.GetAbsolutePath(MediaServicesDefaults.ImageThumbsPath), filter, false);
        ...
        {
            var thumbsDirectoryPath = _fileProvider.GetAbsolutePath(MediaServicesDefaults.ImageThumbsPath);

            if (_mediaSettings.MultipleThumbDirectories)
            {
                //get the first two letters of the file name
                var fileNameWithoutExtension = _fileProvider.GetFileNameWithoutExtension(thumbFileName);
                if (fileNameWithoutExtension != null && fileNameWithoutExtension.Length > MediaServicesDefaults.MultipleThumbDirectoriesLength)
                {
                    var subDirectoryName = fileNameWithoutExtension[0..MediaServicesDefaults.MultipleThumbDirectoriesLength];
                    thumbsDirectoryPath = _fileProvider.GetAbsolutePath(MediaServicesDefaults.ImageThumbsPath, subDirectoryName);
        ...
        /// Get images path URL 
        /// </summary>
        /// <param name="nodeLocation">Node location URL; null to use determine the current node location automatically</param>
        /// <returns>
        /// A task that represents the asynchronous operation
        /// The task result contains the 
        /// </returns>
        protected virtual Task<string> GetImagesPathUrlAsync(string nodeLocation = null)
        {
            var pathBase = _httpContextAccessor.HttpContext.Request?.PathBase.Value ?? string.Empty;
            var imagesPathUrl = _mediaSettings.UseAbsoluteImagePath ? nodeLocation : $"{pathBase}/";
            imagesPathUrl = string.IsNullOrEmpty(imagesPathUrl) ? _webHelper.GetNodeLocation() : imagesPathUrl;
        ...
        /// The task result contains the local picture thumb path
        /// </returns>
        protected virtual async Task<string> GetThumbUrlAsync(string thumbFileName, string nodeLocation = null)
        {
            var url = await GetImagesPathUrlAsync(nodeLocation) + "thumbs/";
            ...
            string nodeLocation = null)
        {
            ...
            if (defaultPictureType == PictureType.Entity && _mediaSettings.GameplayDefaultImageId > 0)
                return await GetPictureUrlAsync(_mediaSettings.GameplayDefaultImageId, targetSize, false, nodeLocation, PictureType.Entity);

            var defaultImageFileName = defaultPictureType switch
            {
                PictureType.Avatar => await _settingService.GetSettingByKeyAsync("Media.User.DefaultAvatarImageName", MediaServicesDefaults.DefaultAvatarFileName),
                _ => await _settingService.GetSettingByKeyAsync("Media.DefaultImageName", MediaServicesDefaults.DefaultImageFileName),
            };
            ...
            if (targetSize == 0)
                return await GetImagesPathUrlAsync(nodeLocation) + defaultImageFileName;
            ...
            return await GetThumbUrlAsync(thumbFileName, nodeLocation);
        }
            ...
            PictureType defaultPictureType = PictureType.Entity)
        {
            ...
            return (await GetPictureUrlAsync(picture, targetSize, showDefaultPicture, nodeLocation, defaultPictureType)).Url;
        }
        ...
        {
            if (picture == null)
                return showDefaultPicture ? (await GetDefaultPictureUrlAsync(targetSize, defaultPictureType, nodeLocation), null) : (string.Empty, null);
            ...
            if (picture.IsNew)
            {
                ...
                if ((pictureBinary?.Length ?? 0) == 0)
                    return showDefaultPicture ? (await GetDefaultPictureUrlAsync(targetSize, defaultPictureType, nodeLocation), picture) : (string.Empty, picture);
            ...
            if (targetSize == 0 || picture.MimeType == MimeTypes.ImageSvg)
            {
                var thumbFilePath = await GetThumbLocalPathAsync(thumbFileName);
                if (await GeneratedThumbExistsAsync(thumbFilePath, thumbFileName))
                    return (await GetThumbUrlAsync(thumbFileName, nodeLocation), picture);
        ...
        ///// <summary>
        ///// Gets pictures by tournament identifier
        ///// </summary>
        ...
        ///// </returns>
        //public virtual async Task<IList<Picture>> GetPicturesByTournamentIdAsync(int tournamentId, int recordsToReturn = 0)
        //{
        //    if (tournamentId == 0)
        //        return new List<Picture>();

        //    var query = from p in _pictureRepository.Table
        //                join pp in _tournamentPictureRepository.Table on p.Id equals pp.PictureId
        //                orderby pp.DisplayOrder, pp.Id
        //                where pp.TournamentId == tournamentId
        //                select p;
        ...
        ///// <summary>
        ///// Gets pictures by quest identifier
        ///// </summary>
        ...
        ///// The task result contains the pictures
        ///// </returns>
        //public virtual async Task<IList<Picture>> GetPicturesByQuestIdAsync(int questId, int recordsToReturn = 0)
        //{
        //    if (questId == 0)
        //        return new List<Picture>();

        //    var query = from p in _pictureRepository.Table
        //                join pp in _questPictureRepository.Table on p.Id equals pp.PictureId
        //                orderby pp.DisplayOrder, pp.Id
        ...
        ///// <summary>
        ///// Gets pictures by game title identifier
        ///// </summary>
        ///// <param name="titleId">Game title identifier</param>
        ///// <param name="recordsToReturn">Number of records to return. 0 if you want to get all items</param>
        ///// <returns>
        ///// A task that represents the asynchronous operation
        ///// The task result contains the pictures
        ///// </returns>
        //public virtual async Task<IList<Picture>> GetPicturesByGameTitleIdAsync(int titleId, int recordsToReturn = 0)
        //{
        //    if (titleId == 0)
        //        return new List<Picture>();

        //    var query = from p in _pictureRepository.Table
        //                join tp in _titlePictureRepository.Table on p.Id equals tp.PictureId
        //                orderby tp.DisplayOrder, tp.Id
