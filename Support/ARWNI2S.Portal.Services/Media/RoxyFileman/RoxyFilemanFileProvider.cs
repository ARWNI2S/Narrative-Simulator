    ...
    public class RoxyFilemanFileProvider : PhysicalFileProvider, IRoxyFilemanFileProvider
    {
        ...
        protected virtual byte[] ResizeImage(byte[] data, SKEncodedImageFormat format, int maxWidth, int maxHeight)
        {
            ...
            if (!image.ScalePixels(toBitmap, SKFilterQuality.None))
                throw new NodeException("Image scaling");
        ...
        /// </summary>
        /// <param name="pathBase">The base path for the server</param>
        ...
        public virtual async Task<RoxyFilemanConfig> GetOrCreateConfigurationAsync(string pathBase, string lang)
        {
            ...
            var filePath = _fileProvider.GetAbsolutePath(RoxyFilemanServiceDefaults.ConfigurationFile);
            ...
            {
                FILES_ROOT = existingConfiguration?.FILES_ROOT ?? RoxyFilemanServiceDefaults.DefaultRootDirectory,
                ...
                FORBIDDEN_UPLOADS = existingConfiguration?.FORBIDDEN_UPLOADS ?? string.Join(" ", RoxyFilemanServiceDefaults.ForbiddenUploadExtensions),
