    ...
    public partial class RoxyFilemanService : IRoxyFilemanService
    {
        ...
        protected virtual bool CanHandleFile(string path)
        {
            ...
            if (!string.IsNullOrEmpty(forbiddenUploads))
                result = !RoxyFilemanServiceRegex().Split(forbiddenUploads).Contains(fileExtension);

            var allowedUploads = roxyConfig.ALLOWED_UPLOADS.Trim().ToLowerInvariant();
            if (string.IsNullOrEmpty(allowedUploads))
                return result;

            return RoxyFilemanServiceRegex().Split(allowedUploads).Contains(fileExtension);
        }
        ...
        {
            if (destinationPath.StartsWith(sourcePath, StringComparison.InvariantCulture))
        ...
        [GeneratedRegex("\\s+")]
        private static partial Regex RoxyFilemanServiceRegex();
