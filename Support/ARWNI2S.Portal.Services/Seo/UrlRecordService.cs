...
namespace ARWNI2S.Portal.Services.Seo
{
    ...
    public partial class UrlRecordService : IUrlRecordService
    {
        ...
        public virtual async Task<UrlRecord> GetBySlugAsync(string slug)
        {
            ...
            var key = _staticCacheManager.PrepareKeyForDefaultCache(SeoServicesDefaults.UrlRecordBySlugCacheKey, slug);
            ...
            return await _staticCacheManager.GetAsync(key, async () =>
            {
                if (_localizationSettings.LoadAllUrlRecordsOnStartup)
                {
                    var lookup = await _staticCacheManager.GetAsync(
                        _staticCacheManager.PrepareKeyForDefaultCache(SeoServicesDefaults.UrlRecordSlugLookupCacheKey),
        ...
        {
            //gradual loading
            var key = _staticCacheManager.PrepareKeyForDefaultCache(SeoServicesDefaults.UrlRecordCacheKey, entityId, entityName, languageId);
            ...
            return await _staticCacheManager.GetAsync(key, async () =>
            {
                ...
                {
                    var lookup = await _staticCacheManager.GetAsync(
                        _staticCacheManager.PrepareKeyForDefaultCache(SeoServicesDefaults.UrlRecordEntityIdLookupCacheKey, languageId),
        ...
        /// <returns>A task that represents the asynchronous operation</returns>
        public virtual async Task SaveSlugAsync<T>(T entity, string slug, int languageId) where T : BaseDataEntity, ISlugSupported
        {
        ...
        /// </returns>
        public virtual async Task<string> ValidateSeNameAsync<T>(T entity, string seName, string name, bool ensureNotEmpty) where T : BaseDataEntity, ISlugSupported
        {
            ArgumentNullException.ThrowIfNull(entity);
