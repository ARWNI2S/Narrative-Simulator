    ...
    public partial class GenericAttributeService : IGenericAttributeService
    {
        ...
        public virtual async Task<IList<GenericAttribute>> GetAttributesForEntityAsync(int entityId, string keyGroup)
        {
            var query = from ga in _genericAttributeRepository.Table
                        ...
                        select ga;
            var attributes = await _shortTermCacheManager.GetAsync(async () => await query.ToListAsync(), CommonDefaults.GenericAttributeCacheKey, entityId, keyGroup);
        ...
        /// <returns>A task that represents the asynchronous operation</returns>
        public virtual async Task SaveAttributeAsync<TPropType>(DataEntity entity, string key, TPropType value, int storeId = 0)
        {
        ...
        /// </returns>
        public virtual async Task<TPropType> GetAttributeAsync<TPropType>(DataEntity entity, string key, int storeId = 0, TPropType defaultValue = default)
        {
            ArgumentNullException.ThrowIfNull(entity);
