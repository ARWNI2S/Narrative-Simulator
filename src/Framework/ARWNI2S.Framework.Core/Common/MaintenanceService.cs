...
namespace ARWNI2S.Framework.Common
{
    ...
    public partial class MaintenanceService : IMaintenanceService
    {
        ...
        {
            var path = _fileProvider.GetAbsolutePath(CommonDefaults.DbBackupsPath);
        ...
        public virtual IList<string> GetAllBackupFiles()
        {
            ...
            return _fileProvider.GetFiles(path, $"*.{CommonDefaults.DbBackupFileExtension}")
        ...
        {
            return _fileProvider.Combine(GetBackupDirectoryPath(), $"database_{DateTime.Now:yyyy-MM-dd-HH-mm-ss}_{CommonHelper.GenerateRandomDigitCode(10)}.{CommonDefaults.DbBackupFileExtension}");
        }
