...
namespace ARWNI2S.Framework.Plugins
{
    /// <summary>
    ...
    public partial class PluginService : IPluginService
    {
        ...
        public virtual Task<string> GetPluginLogoUrlAsync(PluginDescriptor pluginDescriptor)
        {
            ...
            var logoExtension = PluginDefaults.SupportedLogoImageExtensions
                .FirstOrDefault(ext => _fileProvider.FileExists(_fileProvider.Combine(pluginDirectory, $"{PluginDefaults.LogoFileName}.{ext}")));
            ...
            var logoUrl = $"{logoPathUrl}{PluginDefaults.PathName}/" +
                          $"{_fileProvider.GetDirectoryNameOnly(pluginDirectory)}/{PluginDefaults.LogoFileName}.{logoExtension}";
            ...
            if (checkDependencies)
            {
                ...
                if (descriptor.DependsOn?.Any() ?? false)
                {
                    ...
                    if (dependsOn.Any())
                    {
                        ...
                        throw new NI2SException(errorMessage);
                    }
                }
            }
        ...
        public virtual async Task UpdatePluginsAsync()
        {
            ...
            foreach (var installedPlugin in _pluginsInfo.InstalledPlugins)
            {
                ...
                //activity log                
                await userActivityService.InsertActivityAsync("UpdatePlugin",
